...
<script type="text/javascript">
	...
	function getStage() {
		const pMap = JSON.parse('${applicationScope.pMap}');
		let stage = $("#stage").html();
		$("#possibility").html(pMap[stage]);

		const dicMap = JSON.parse('${applicationScope.dicMap}');
		const stageList = dicMap.stageList;
		//当前阶段下标
		$.each(stageList,function (i,n) {
			if (stage === n.text){
				curStageInd = i;
				return false;
			}
		...
		let html = "阶段&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
		$.each(stageList,function (i,n) {
			if (curStageInd < passPoint){
				if (i<curStageInd){
					html += '<span class="glyphicon glyphicon-ok-circle mystage" onclick="changeStage(`'+ n.value +'`)" data-toggle="popover" data-placement="bottom" data-content="'+ n.text +'" style="color: #90F790;"></span>';
					html += '-----------';
					//alert(1)
				}
				if (i === curStageInd){
					html += '<span class="glyphicon glyphicon-map-marker mystage" onclick="changeStage(`'+ n.value +'`)" data-toggle="popover" data-placement="bottom" data-content="'+ n.text +'" style="color: #90F790;"></span>';
					html += '-----------';
					//alert(2)
				}
				if (i> curStageInd && i < passPoint){
					html += '<span class="glyphicon glyphicon-record mystage" onclick="changeStage(`'+ n.value +'`)" data-toggle="popover" data-placement="bottom" data-content="'+ n.text +'" style="color: #000000;"></span>';
					html += '-----------';
					//alert(3)
				}
				if (i >= passPoint){
					html += '<span class="glyphicon glyphicon-remove mystage" onclick="changeStage(`'+ n.value +'`)" data-toggle="popover" data-placement="bottom" data-content="'+ n.text +'" style="color: #000000;"></span>';
					html += '-----------';
					//alert(4)
				}
			}else {
				...
				if (i === curStageInd){
					html += '<span class="glyphicon glyphicon-remove mystage" onclick="changeStage(`'+ n.value +'`)" data-toggle="popover" data-placement="bottom" data-content="'+ n.text +'" style="color: #FF0000;"></span>';
					html += '-----------';
					//alert(2)
				}
			}
		});
		html += '<span class="closingDate">2010-10-10</span>';
		$("#stageDiv").html(html);
	}
